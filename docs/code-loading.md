...
Les codes proviennent de `CompIde.data`, le tableau dynamique généré par le backend Node.js (`server.js`).
...
Ce tableau est chargé de manière asynchrone par `js/app.js` via un appel réseau `fetch('/api/data')` au démarrage.
...
1. Chargés asynchronement via `fetch('/api/data')` qui initialise `CompIde.data`
